...
class CB513DatasetProcessor:
    ...
    def process(self):
        ...
        self.input_seqs = [seq.replace(" ", "") for seq in df["input"].tolist()]
        self.dssp8_seqs = [seq.replace(" ", "") for seq in df["dssp8"].tolist()]
        ...
        for i in range(len(self.input_seqs)):
            ...
            if invalid_aa:
                # print(
                #     f"Warning: {invalid_aa}"
                # )
                for char in invalid_aa:
                    # print(
                    #     f"Warning: {invalid_ss}"
                    # )
        ...
        self.stats = {
            "n_proteins": len(self.input_seqs),
    ...
    def load(pt_path: str):
        ...
        processor = CB513DatasetProcessor.__new__(CB513DatasetProcessor)
...
def main():
    csv_path = "cb513_data/cb513_dataset.csv"
    save_path = "cb513_data/cb513_dataset.pt"
    sample_index = 0

    # Cleaning the CSV into a PyTorch object
    start = time.perf_counter()
    processor = CB513DatasetProcessor(csv_path)
    processor.process()
    ...
    loaded_processor = CB513DatasetProcessor.load(save_path)
